# 📊 OCR Benchmark Report (AlphaXiv)

**Generated**: 2025-11-03 18:18:42
**PDF**: /tmp/test_data/Capital414_presentacion.pdf
**Pages Tested**: [1, 2]

## Performance Summary

| Strategy | Success Rate | Avg Duration | Total Chars | Total Words | Status |
|----------|--------------|--------------|-------------|-------------|--------|
| Saptiva OCR | 2/2 (100.0%) | 3752.5ms | 2,863 | 407 | ✅ Success |
| DeepSeek OCR (AlphaXiv) | 2/2 (100.0%) | 17475.1ms | 89 | 13 | ✅ Success |

## Text Similarity (Accuracy)

| Comparison | Similarity |
|------------|------------|
| Saptiva OCR vs DeepSeek OCR (AlphaXiv) | 2.84% |

## 🏆 Best Performers

**Fastest**: Saptiva OCR (7504.97ms total)
**Most Text Extracted**: Saptiva OCR (2,863 chars)

## 💡 Recommendation

❌ **LOW SIMILARITY** - Strategies produce significantly different results

## Detailed Results per Page

### Saptiva OCR

| Page | Chars | Words | Duration | Status |
|------|-------|-------|----------|--------|
| 1 | 237 | 19 | 194.39ms | ✅ |
| 2 | 2626 | 388 | 7309.48ms | ✅ |

### DeepSeek OCR (AlphaXiv)

| Page | Chars | Words | Duration | Status |
|------|-------|-------|----------|--------|
| 1 | 89 | 13 | 2121.49ms | ✅ |
| 2 | 0 | 0 | 32827.58ms | ✅ |
