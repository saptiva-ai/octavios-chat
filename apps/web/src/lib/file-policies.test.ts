...
describe("file-policies", () => {
  ...
  describe("shouldLoadDocumentsFromBackend", () => {
    ...
    it("should NOT load for temp chats", () => {
      expect(shouldLoadDocumentsFromBackend(tempChatId, false, true)).toBe(false);
      expect(shouldLoadDocumentsFromBackend(tempChatId, true, true)).toBe(false);
    });

    it("should NOT load for creating chats", () => {
      expect(shouldLoadDocumentsFromBackend(creatingChatId, false, true)).toBe(false);
    });

    it("should NOT load for real chats with messages", () => {
      expect(shouldLoadDocumentsFromBackend(testChatId, true, true)).toBe(false);
    });

    it("should load for real empty chats (hydrated)", () => {
      expect(shouldLoadDocumentsFromBackend(testChatId, false, true)).toBe(true);
    });

    it("should NOT load for real chats not yet hydrated", () => {
      expect(shouldLoadDocumentsFromBackend(testChatId, false, false)).toBe(false);
      expect(shouldLoadDocumentsFromBackend(testChatId, true, false)).toBe(false);
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    it("handles special characters in chatId", () => {
      ...
      expect(shouldRestoreFiles("chat_with_underscores", false, true)).toBe(true);
    });
