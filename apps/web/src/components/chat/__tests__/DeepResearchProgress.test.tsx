...
import type { ResearchPhase } from '../../../hooks/useDeepResearch'
...
describe('DeepResearchProgress', () => {
  ...
  describe('Sources Display', () => {
    ...
    it('shows clickable source URLs', () => {
      ...
      expect(links[1]).toHaveAttribute('href', 'https://example.com/fintech-report')
    })

    it('displays message when no sources available', () => {
      render(<DeepResearchProgress {...defaultProps} sources={[]} />)
      expect(screen.getByText('Aún no se han agregado fuentes.')).toBeInTheDocument()
    })
  ...
  describe('Evidence Display', () => {
    it('renders evidence claims correctly', () => {
      render(<DeepResearchProgress {...defaultProps} />)
      expect(screen.getByText('AI adoption has increased 150% in LATAM')).toBeInTheDocument()
      expect(screen.getByText('FinTech investment reached $2.1B in 2024')).toBeInTheDocument()
    })

    it('shows confidence levels', () => {
      render(<DeepResearchProgress {...defaultProps} />)
      expect(screen.getByText('Confianza: strong')).toBeInTheDocument()
      expect(screen.getByText('Confianza: moderate')).toBeInTheDocument()
    })

    it('displays message when no evidence available', () => {
      render(<DeepResearchProgress {...defaultProps} evidences={[]} />)
      expect(screen.getByText('Se están evaluando hallazgos relevantes.')).toBeInTheDocument()
    })
    ...
    it('limits evidence display to top 3', () => {
      ...
      render(<DeepResearchProgress {...defaultProps} evidences={manyEvidences} />)
  ...
  describe('Report Display', () => {
    it('shows TL;DR when report is available', () => {
      ...
      expect(screen.getByText('AI adoption growing rapidly with strong economic impact')).toBeInTheDocument()
    })

    it('falls back to summary if no TL;DR', () => {
      const reportWithoutTldr = { ...mockReport, tl_dr: undefined }
      render(<DeepResearchProgress {...defaultProps} report={reportWithoutTldr} />)
      expect(screen.getByText('AI has significant impact on LATAM economy')).toBeInTheDocument()
    })
  ...
  describe('Error Handling', () => {
    it('displays error message when provided', () => {
      ...
      render(<DeepResearchProgress {...defaultProps} errorMessage={errorMessage} />)
      expect(screen.getByText(errorMessage)).toBeInTheDocument()
    })

    it('shows default error message for failed phase', () => {
      render(<DeepResearchProgress {...defaultProps} phase="FAILED" />)
      expect(screen.getByText(/No se pudo completar la investigación/)).toBeInTheDocument()
    })
    ...
    it('hides sources and evidence sections when there is an error', () => {
      render(<DeepResearchProgress {...defaultProps} errorMessage="Test error" />)
      expect(screen.queryByText('Fuentes recientes')).not.toBeInTheDocument()
      expect(screen.queryByText('Evidencias destacadas')).not.toBeInTheDocument()
    })
  })
  ...
  describe('Action Buttons', () => {
    ...
    it('calls onClose when close button is clicked', () => {
      ...
      render(<DeepResearchProgress {...defaultProps} phase="COMPLETED" onClose={onClose} />)
