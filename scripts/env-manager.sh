...
check_environment() {
    ...
    if [[ ${#missing_vars[@]} -gt 0 ]]; then
        echo -e "${RED}⚠️  Missing variables: ${missing_vars[*]}${NC}"
        return 1
    else
...
switch_environment() {
    ...
    if docker ps | grep -q "copilotos-"; then
        ...
        make restart > /dev/null 2>&1 || echo -e "${YELLOW}⚠️  Could not restart services automatically${NC}"
    fi
}
...
validate_environment() {
    ...
    if [[ "$env" == "prod" ]]; then
        ...
        if grep -q "localhost" "$env_file"; then
            echo -e "${YELLOW}⚠️  Found localhost URLs in production${NC}"
        fi
...
diff_environments() {
    ...
    diff -u "$file1" "$file2" || echo -e "${YELLOW}⚠️  Environments differ${NC}"
}
