...
delete_demo_user() {
    echo -e "${YELLOW}🗑️  Checking if demo user needs to be recreated...${NC}"
...
EOF
    ...
    if [ "$LOGIN_CODE" = "200" ]; then
        ...
        echo -e "${YELLOW}⚠️  Login test failed (HTTP $LOGIN_CODE)${NC}"
...
trap 'echo -e "\n${YELLOW}⚠️  Script interrupted${NC}"; exit 1' INT TERM
